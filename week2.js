...
function pyramidCounting(j) {
    let sum = 0; {
        for (let i = 0; i <= j; i++) (sum += i)
    }
    return sum;
}
console.log("Q2: ", pyramidCounting(4));
...
function noVowels(str) {
    ...
    for (let idx in newStr) {
        if (!vowels.includes(newStr[idx].toLowerCase())) {
            del += newStr[idx]
        }
    }
    return del
...
function vowelCount(q4Input) {
    const q4Output = Array.from(q4Input);
    const vowels = ["a", "e", "i", "o", "u"];
    let del = "";
    for (let vowelCount in q4Input) {
        if (vowels.includes(q4Output[vowelCount].toLowerCase())) {
            del += q4Output[vowelCount]
        }
    }
    return del.length
}
console.log("Q4: ", vowelCount("I love Javascript"));
...
function numOfOdds(q5) {
    let q5Count = 0; for (let i = 0; i <= q5; i++)
        if (i % 2 !== 0) {
            q5Count++;
        }
    return q5Count
...
function calculateTotal(numbers) {
    let total = 0;
    numbers.forEach(currentElement => { total += currentElement })
    return total;
}
console.log("Q10: ", calculateTotal(numbers));
...
function fizzBuzz() {
    const q14Array = []
    for (let i = 1; i <= 15; i++) {
        if (i % 15 === 0) q14Array.push("fizzbuzz")
        else if (i % 3 === 0) q14Array.push("fizz")
        else if (i % 5 === 0) q14Array.push("buzz")
        else q14Array.push(i)
    }
    return q14Array
};
